...
export function registerTestFlightCommands(program: Command, env: EnvConfig): void {
  const tf = program
    .command("testflight")
    .description("Manage TestFlight landing page");

  tf
    .command("get")
    .description("Get TestFlight page config")
    ...
    .action(async (argAppId?: string) => {
      ...
      const res = await client.get(`/api/apps/${encodeURIComponent(appId)}/testflight-page`);
      printData(assertOk(res), env.json);
    });

  tf
    .command("set")
    .description("Upload TestFlight page config from JSON file")
    .argument("[appId]", "App ID (or set ORBITKIT_APP_ID)")
    ...
    .action(async (argAppId: string, file?: string) => {
      ...
      const client = new OrbitKitClient(env);
      const res = await client.put(`/api/apps/${encodeURIComponent(appId)}/testflight-page`, data);
      assertOk(res);
      printSuccess("TestFlight page updated.");
    });
}
